...
func (h *executorHandle) EnqueueTaskReservation(ctx context.Context, req *scpb.EnqueueTaskReservationRequest, waitForResponse bool) error {
	...
	// Prior to PR123, the JWT was populated from the context which could either
	// be the user jwt (in the normal path) or the executor jwt (if this function was
	// called from the execution work stream handler).
	// This check maintains this behavior during rollouts from versions that didn't
	// yet have this change.
	if req.GetJwt() == "" {
